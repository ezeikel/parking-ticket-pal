...
export const access = async ({ page, pcnNumber, ticket }: CommonPcnArgs) => {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt += 1) {
    try {
      await page.goto('https://www.westminster.gov.uk/parking/pcn');

  // Fill Enter your ticket reference
  await page.fill('[2-4]', /* TODO: Map field "ticket_reference" */);
  // Fill Enter your vehicle registration number
  await page.fill('[2-5]', ticket.vehicle.registrationNumber);

      // Submit lookup form
      await page.click('[2-20]');
      ...
      if (attempt === maxRetries) {
        Sentry.captureException(error, {
          tags: { issuer: 'westminster-test', action: 'access' },
        });
        throw error;
      }
...
export const challenge = async (
  ...
  // Enter PCN reference number in the ticket reference field
  // Enter vehicle registration number
  await page.click('[2-20]'); // Click the Show ticket button to proceed
